...
class VanillaGradientDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        :return: weight difference (w_{k + 1} - w_k): np.ndarray
        """
        w_difference = -self.lr() * gradient
        self.w = self.w + w_difference
        return w_difference

    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        if self.loss_function.name == LossFunction.MSE.name:
            return -2 * np.array((y - x.dot(self.w)).T).dot(x) / x.shape[0]
        elif self.loss_function.name == LossFunction.LogCosh.name:
            return np.tanh(x.dot(self.w) - y).T.dot(x) / x.shape[0]
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function.name == LossFunction.MSE.name:
            return -2 * (y[ind] - x[ind].dot(self.w)).T.dot(x[ind]) / self.batch_size
        elif self.loss_function.name == LossFunction.LogCosh.name:
            return np.tanh(x[ind].dot(self.w) - y[ind]).T.dot(x[ind]) / self.batch_size
